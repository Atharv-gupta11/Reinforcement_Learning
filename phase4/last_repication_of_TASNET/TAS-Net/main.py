...
from models import DSN, MambaDSN, SpatialAgent
...
if __name__ == '__main__':
    if args.training:
        ...
        Network1 = ClassifierGNN(in_features=args.n_feature,
                                 ...
                                 device=DEVICE)
        Network2 = MambaDSN(in_dim=args.n_feature, hid_dim=args.hid_dim, num_layers=1)
        NetworkFeature = SpatialAgent(in_dim=args.n_feature, hid_dim=64)
        # Network2 = FC(in_dim=192)
        Network2.apply(weights_init)
        NetworkFeature.apply(weights_init)
        print("Model size (Network1): {:.5f}M".format(sum(p.numel() for p in Network1.parameters()) / 1000000.0))
        print("Model size (Network2): {:.5f}M".format(sum(p.numel() for p in Network2.parameters()) / 1000000.0))
        print("Model size (NetworkFeature): {:.5f}M".format(sum(p.numel() for p in NetworkFeature.parameters()) / 1000000.0))
        parameter_list = [{'params': Network1.parameters(), 'lr_mult': 10, 'decay_mult': 2}] + \
                         [{'params': Network2.parameters(), 'lr_mult': 10, 'decay_mult': 2}] + \
                         [{'params': NetworkFeature.parameters(), 'lr_mult': 10, 'decay_mult': 2}]
        ...
        NetworkFeature = NetworkFeature.to(DEVICE)

        print("=====> Start MARL training and Online Evaluate <===== ")
        start_time = time.time()
        Network1.train()
        Network2.train()
        NetworkFeature.train()
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            for idx in idxs:
                ...
                # --- Phase 3: Differentiable Bridge (Gumbel-Softmax) ---
                s_probs = NetworkFeature(seq) # [1, 1, 192]
                
                # Convert to logits to use Gumbel-Softmax
                logits_0 = torch.log(1.0 - s_probs + 1e-8)
                logits_1 = torch.log(s_probs + 1e-8)
                s_logits = torch.stack([logits_0, logits_1], dim=-1) # [1, 1, 192, 2]
                
                # Sparsity & Density Penalities to STOP Agent Collusion
                s_prob_mean = s_probs.mean()
                sparsity_penalty = torch.relu(0.3 - s_prob_mean) * 20.0  # Force it to keep >30% features
                density_penalty = torch.relu(s_prob_mean - 0.7) * 10.0   # Force it to drop some features
                cost = sparsity_penalty + density_penalty
                ...
                for episode in range(20):
                    # Spatial Agent Action (Differentiable via Gumbel)
                    s_actions_onehot = F.gumbel_softmax(s_logits, tau=0.5, hard=True)
                    s_actions = s_actions_onehot[..., 1] # [1, 1, 192]
                    
                    # Mask sequence using Spatial Agent's differentiable choice
                    masked_seq = seq * s_actions
                    
                    # Temporal Agent acts on masked sequence
                    t_probs = Network2(masked_seq)
                    t_m = Bernoulli(t_probs)
                    t_actions = t_m.sample()
                    t_log_probs = t_m.log_prob(t_actions).sum()
                    
                    # Phase 4: Entropy Regularization
                    # Mathematically prevents T-Agent from collapsing its probabilities to 1.0 or 0.0
                    t_entropy = -(t_probs * torch.log(t_probs + 1e-8) + (1.0 - t_probs) * torch.log(1.0 - t_probs + 1e-8)).mean()
                    
                    reward = compute_reward(seq, t_actions)
                    
                    # Since S-Agent is bridged differentiably, T-Agent's log_probs gradient 
                    # will flow naturally backwards through the mask! No s_log_probs needed.
                    ep_log_probs.append(t_log_probs)
                    ep_rewards.append(reward)
                    ep_entropies.append(t_entropy)

                rewards_tensor = torch.stack(ep_rewards)
                reward_mean = rewards_tensor.mean()
                reward_std = rewards_tensor.std() + 1e-8

                for log_probs, reward in zip(ep_log_probs, ep_rewards):
                    normalized_reward = (reward - reward_mean) / reward_std
                    expected_reward = log_probs.sum() * normalized_reward
                    cost -= expected_reward / 20.0
                
                # --- Phase 4: RL Entropy Bonus ---
                # Subtracting entropy from the cost (which PyTorch minimizes) forces the network 
                # to MAXIMIZE entropy, keeping it out of the 0.0 variance coma!
                mean_entropy = torch.stack(ep_entropies).mean()
                cost -= 0.05 * mean_entropy

                baselines[key] = 0.9 * baselines[key] + 0.1 * reward_mean.item()
                reward_writers[key].append(reward_mean.item())

                optimizer.zero_grad()
                cost.backward()
                torch.nn.utils.clip_grad_norm_(Network1.parameters(), 5.0)
                torch.nn.utils.clip_grad_norm_(Network2.parameters(), 5.0)
                torch.nn.utils.clip_grad_norm_(NetworkFeature.parameters(), 5.0)
                optimizer.step()
            ...
            if (epoch + 1) % 1 == 0:
                Recall = evaluate(args, Network1, Network2, NetworkFeature, datasets, test_keys)
                print("\n" + "="*50)
                print(f"[ EVALUATION ] Epoch {epoch+1}/{args.epochs} => Recall: {Recall:.4f}")
                print("="*50 + "\n")
                if Recall > best_recall:
                    best_recall = Recall
                    model_state_dict1 = Network1.state_dict()
                    model_state_dict2 = Network2.state_dict()
                    model_state_dict3 = NetworkFeature.state_dict()
                    ...
                    model_save_path3 = osp.join(args.save_path,
                                                'pretrained_best_model3_seed_' + 'subject' + str(args.subject_id) + '_' + str(args.reward_function) + '.pth.tar')
                    save_checkpoint(model_state_dict1, model_save_path1)
                    save_checkpoint(model_state_dict2, model_save_path2)
                    save_checkpoint(model_state_dict3, model_save_path3)
        ...
        model1 = ClassifierGNN(in_features=args.n_feature,
                                 ...
                                 out_features=args.n_feature,
                                 device=DEVICE)
        model2 = MambaDSN(in_dim=args.n_feature, hid_dim=args.hid_dim, num_layers=1)
        modelFeature = SpatialAgent(in_dim=args.n_feature, hid_dim=64)
        ...
        checkpoint_path3 = osp.join(args.save_path, 'pretrained_best_model3_seed_' + 'subject' + str(args.subject_id) + '_' + str(args.reward_function) + '.pth.tar')
        checkpoint3 = torch.load(checkpoint_path3)
        modelFeature.load_state_dict(checkpoint3)
        modelFeature = modelFeature.to(DEVICE)
        with torch.no_grad():
            model1.eval()
            model2.eval()
            modelFeature.eval()
            ...
            for key_idx, key in enumerate(test_keys):
                ...
                # --- Spatial Agent Eval ---
                s_probs_infer = modelFeature(seq2seq)
                s_actions_infer = (s_probs_infer > 0.5).float()
                masked_seq2seq = seq2seq * s_actions_infer
                
                sig_probs = model2(masked_seq2seq)

                probs_importance = sig_probs.data.cpu().squeeze().numpy()
                probs_importance = np.convolve(probs_importance, [0.15, 0.7, 0.15], mode='same')

                seq = seq.squeeze()
                # limits = int(math.floor(seq.shape[0] * prop))
                limits = args.num_fragment
                
                # STAGE 2 FIX: Non-Maximum Suppression (NMS)
                # Stops the evaluation from clumping exactly adjacent frames together,
                # which ruins recall. Forces predictions to be at least 8 frames apart.
                order_raw = np.argsort(probs_importance)[::-1]
                keep = []
                min_dist = 8 
                for idx in order_raw:
                    if all(abs(idx - k) > min_dist for k in keep):
                        keep.append(idx)
                    if len(keep) == limits: break
                        
                # Auto-pad if couldn't find enough distant fragments
                if len(keep) < limits:
                    for idx in order_raw:
                        if idx not in keep: keep.append(idx)
                        if len(keep) == limits: break
