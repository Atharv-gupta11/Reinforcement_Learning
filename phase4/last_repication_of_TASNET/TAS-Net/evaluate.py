...
def evaluate(args, model1, model2, modelFeature, dataset, test_keys):
    ...
    with torch.no_grad():
        ...
        for key_idx, key in enumerate(test_keys):
            ...
            # --- Spatial Agent Eval ---
            s_probs = modelFeature(seq_graph0)
            # Hard mask during evaluation
            s_actions = (s_probs > 0.5).float()
            masked_seq = seq_graph0 * s_actions

            sig_probs = model2(masked_seq)

            probs_importance = sig_probs.data.cpu().squeeze().numpy()
            # Temporal smoothing: dampens spiky noise peaks so NMS picks
            # coherent target intervals rather than isolated noise frames.
            ...
            keep = []
            min_dist = 5  # reduced from 8 to allow denser GT regions to be covered
